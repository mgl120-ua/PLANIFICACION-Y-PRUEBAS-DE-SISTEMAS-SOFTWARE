...
public class AlquilaCoches {
        ...
        public Ticket calculaPrecio(TipoCoche tipo, LocalDate inicio, int ndias) throws MensajeException {
            ...
            //IService servicio = new Servicio(); -> REFACTORIZACIÓN
            IService servicio = getServicio();
            precioDia = servicio.consultaPrecio(tipo); //DEPENDENCIA
            for (int i=0; i<ndias;i++) {
                LocalDate otroDia = inicio.plusDays((long)i); //devuelve la fecha resultante de añadir "i" dias a la fecha "inicio"
                try {
